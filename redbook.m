...
T = 30;   %keraunic level tunderstorm days per year
...
NL = N*((X(2)-X(1))+4*hg^1.09)/10;  % Line Flashes per 100km per year Eq. 12.4.6
%NL = 6*(28*TW^0.6+12.5)/10;
...
Rg = 20;    %ohm impulse footing resistance Fig. 12.6.1
...
title('IC fluctuation per each phase')
ylabel('Icn [kA]')
xlabel('angle [°]')
...
grid on
title('Average Icn per phase dominated')
xlabel('angle [°]')
ylabel('Icn [kA]')
axis([0 360 122 136])
xticks(0:30:360)
